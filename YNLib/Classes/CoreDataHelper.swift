...
open class CoreDataHelper: NSObject {
    ...
    fileprivate class func getEntityName<T: NSManagedObject>(_ entityClass: T.Type) -> String {
        let nameSpaceClassName = NSStringFromClass(T.self)
    ...
    // for non generic support Objective c
    public class func createEntityForClass(_ clazz: NSManagedObject.Type, inContext: NSManagedObjectContext) -> NSManagedObject {
        return NSEntityDescription.insertNewObject(forEntityName: NSStringFromClass(clazz), into: inContext)
    }
